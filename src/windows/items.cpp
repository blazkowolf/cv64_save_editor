...
void ItemsForm::setup()
{
    ...
    LineEditHelper::setup(m_ui->leItemsSpecial1, QString::number(0), validator, [this](const QString &text) {
        ...
        SaveManager::getInstance()->setItem(Save::ITEM_ID_SPECIAL1, value);
    });

    // LineEditHelper::setup(ui->leItemsSpecial2, 0, 1,
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_SPECIAL2, value);
    //     }
    // );
    ...
    //             SaveManager::getInstance()->setItem(Save::ITEM_ID_SPECIAL3, value);
    //         }
    //     }
    // );
    LineEditHelper::setup(m_ui->leItemsSpecial3, QString::number(0), validator, [this](const QString &text) {
        ...
        // This item is version exclusive. Ensure we're only setting it when its associated region is set.
        if (auto *inst = SaveManager::getInstance(); inst->getRegion() == Save::PAL || inst->getRegion() == Save::JPN) {
            inst->setItem(Save::ITEM_ID_SPECIAL3, value);
        }
    });
    ...
    //         std::int32_t actualItemId = Save::ITEM_ID_ROAST_CHICKEN;
    //         if (SaveManager::getInstance()->getRegion() == Save::PAL ||
    //             SaveManager::getInstance()->getRegion() == Save::JPN) {
    //             actualItemId++;
    //         }
    ...
    LineEditHelper::setup(m_ui->leItemsRoastChicken, QString::number(0), validator, [this](const QString &text) {
        ...
        // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
        inst->setItem(actualItemId(inst, Save::ITEM_ID_ROAST_CHICKEN), value);
    });

    // LineEditHelper::setup(ui->leItemsRoastBeef, 0, 10,
    //     [](std::uint8_t value) {
    //         // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
    //         std::int32_t actualItemId = Save::ITEM_ID_ROAST_BEEF;
    //         if (SaveManager::getInstance()->getRegion() == Save::PAL ||
    //             SaveManager::getInstance()->getRegion() == Save::JPN) {
    //             actualItemId++;
    //         }
    ...
    LineEditHelper::setup(m_ui->leItemsRoastBeef, QString::number(0), validator, [this](const QString &text) {
        ...
        auto *inst = SaveManager::getInstance();
        // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
        inst->setItem(actualItemId(inst, Save::ITEM_ID_ROAST_BEEF), value);
    });

    // LineEditHelper::setup(ui->leItemsPurifying, 0, 10,
    //     [](std::uint8_t value) {
    //         // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
    //         std::int32_t actualItemId = Save::ITEM_ID_PURIFYING;
    //         if (SaveManager::getInstance()->getRegion() == Save::PAL ||
    //             SaveManager::getInstance()->getRegion() == Save::JPN) {
    //             actualItemId++;
    //         }
    ...
    LineEditHelper::setup(m_ui->leItemsPurifying, QString::number(0), validator, [this](const QString &text) {
        ...
        auto *inst = SaveManager::getInstance();
        // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
        inst->setItem(actualItemId(inst, Save::ITEM_ID_PURIFYING), value);
    });

    // LineEditHelper::setup(ui->leItemsCureAmpoule, 0, 10,
    //     [](std::uint8_t value) {
    //         // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
    //         std::int32_t actualItemId = Save::ITEM_ID_CURE_AMPOULE;
    //         if (SaveManager::getInstance()->getRegion() == Save::PAL ||
    //             SaveManager::getInstance()->getRegion() == Save::JPN) {
    //             actualItemId++;
    //         }
    ...
    LineEditHelper::setup(m_ui->leItemsCureAmpoule, QString::number(0), validator, [this](const QString &text) {
        ...
        auto *inst = SaveManager::getInstance();
        // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
        inst->setItem(actualItemId(inst, Save::ITEM_ID_CURE_AMPOULE), value);
    });
    ...
    //             SaveManager::getInstance()->setItem(Save::ITEM_ID_POUT_POURRI, value);
    //         }
    //     }
    // );
    LineEditHelper::setup(m_ui->leItemsPoutPourri, QString::number(0), validator, [this](const QString &text) {
        ...
        // This item is version exclusive. Ensure we're only setting it when its associated region is set.
        if (auto *inst = SaveManager::getInstance(); inst->getRegion() == Save::USA) {
            inst->setItem(Save::ITEM_ID_POUT_POURRI, value);
        }
    });
    ...
    //     [](std::uint8_t value) {
    //         // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
    //         std::int32_t actualItemId = Save::ITEM_ID_HEALING_KIT;
    //         if (SaveManager::getInstance()->getRegion() == Save::PAL ||
    //             SaveManager::getInstance()->getRegion() == Save::JPN) {
    //             actualItemId++;
    //         }
    ...
    LineEditHelper::setup(m_ui->leItemsHealingKit, QString::number(0), validator, [this](const QString &text) {
        ...
        auto *inst = SaveManager::getInstance();
        // In the JPN and PAL versions, this item's ID is the same as USA, but added +1.
        inst->setItem(actualItemId(inst, Save::ITEM_ID_HEALING_KIT), value);
    });

    // Quest Items
    // LineEditHelper::setup(ui->leItemsSunCard, 0, 10,
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_SUN_CARD, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_MOON_CARD, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_MAGICAL_NITRO, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_MANDRAGORA, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_SCIENCE_KEY1, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_SCIENCE_KEY2, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_SCIENCE_KEY3, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_CLOCKTOWER_KEY1, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_CLOCKTOWER_KEY2, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_CLOCKTOWER_KEY3, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_CHAMBER_KEY, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_COPPER_KEY, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_EXECUTION_KEY, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_GARDEN_KEY, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_LEFT_TOWER_KEY, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_ARCHIVES_KEY, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_STOREROOM_KEY, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_ENGAGEMENT_RING, value);
    //     }
    // );
    ...
    //     [](std::uint8_t value) {
    //         SaveManager::getInstance()->setItem(Save::ITEM_ID_INCANDESCENT_GAZE, value);
    //     }
    // );
...
void ItemsForm::populate(SaveData *saveData) const
{
    ...
    m_ui->leItemsSpecial1->setText(QString::number(saveData->getItem(Save::ITEM_ID_SPECIAL1)));
    m_ui->leItemsSpecial2->setText(QString::number(saveData->getItem(Save::ITEM_ID_SPECIAL2)));

    if (SaveManager::getInstance()->getRegion() == Save::PAL ||
        SaveManager::getInstance()->getRegion() == Save::JPN) {
        m_ui->leItemsSpecial3->setText(QString::number(saveData->getItem(Save::ITEM_ID_SPECIAL3)));
    }
    else {
    ...
    if (SaveManager::getInstance()->getRegion() == Save::USA) {
        m_ui->leItemsRoastChicken->setText(QString::number(saveData->getItem(Save::ITEM_ID_ROAST_CHICKEN)));
        m_ui->leItemsRoastBeef->setText(QString::number(saveData->getItem(Save::ITEM_ID_ROAST_BEEF)));
        m_ui->leItemsHealingKit->setText(QString::number(saveData->getItem(Save::ITEM_ID_HEALING_KIT)));
        m_ui->leItemsPurifying->setText(QString::number(saveData->getItem(Save::ITEM_ID_PURIFYING)));
        m_ui->leItemsCureAmpoule->setText(QString::number(saveData->getItem(Save::ITEM_ID_CURE_AMPOULE)));
    }
    else {
        m_ui->leItemsRoastChicken->setText(QString::number(saveData->getItem(Save::ITEM_ID_ROAST_CHICKEN + 1)));
        m_ui->leItemsRoastBeef->setText(QString::number(saveData->getItem(Save::ITEM_ID_ROAST_BEEF + 1)));
        m_ui->leItemsHealingKit->setText(QString::number(saveData->getItem(Save::ITEM_ID_HEALING_KIT + 1)));
        m_ui->leItemsPurifying->setText(QString::number(saveData->getItem(Save::ITEM_ID_PURIFYING + 1)));
        m_ui->leItemsCureAmpoule->setText(QString::number(saveData->getItem(Save::ITEM_ID_CURE_AMPOULE + 1)));
    }

    if (SaveManager::getInstance()->getRegion() == Save::USA) {
        m_ui->leItemsPoutPourri->setText(QString::number(saveData->getItem(Save::ITEM_ID_POUT_POURRI)));
    }
    else {
        m_ui->leItemsPoutPourri->setText("0");
    }

    m_ui->leItemsSunCard->setText(QString::number(saveData->getItem(Save::ITEM_ID_SUN_CARD)));
    m_ui->leItemsMoonCard->setText(QString::number(saveData->getItem(Save::ITEM_ID_MOON_CARD)));
    m_ui->leItemsNitro->setText(QString::number(saveData->getItem(Save::ITEM_ID_MAGICAL_NITRO)));
    m_ui->leItemsMandragora->setText(QString::number(saveData->getItem(Save::ITEM_ID_MANDRAGORA)));
    m_ui->leKeyArchives->setText(QString::number(saveData->getItem(Save::ITEM_ID_ARCHIVES_KEY)));
    m_ui->leKeyLeftTower->setText(QString::number(saveData->getItem(Save::ITEM_ID_LEFT_TOWER_KEY)));
    m_ui->leKeyStoreroom->setText(QString::number(saveData->getItem(Save::ITEM_ID_STOREROOM_KEY)));
    m_ui->leKeyGarden->setText(QString::number(saveData->getItem(Save::ITEM_ID_GARDEN_KEY)));
    m_ui->leKeyCopper->setText(QString::number(saveData->getItem(Save::ITEM_ID_COPPER_KEY)));
    m_ui->leKeyChamber->setText(QString::number(saveData->getItem(Save::ITEM_ID_CHAMBER_KEY)));
    m_ui->leKeyExecution->setText(QString::number(saveData->getItem(Save::ITEM_ID_EXECUTION_KEY)));
    m_ui->leKeyScience1->setText(QString::number(saveData->getItem(Save::ITEM_ID_SCIENCE_KEY1)));
    m_ui->leKeyScience2->setText(QString::number(saveData->getItem(Save::ITEM_ID_SCIENCE_KEY2)));
    m_ui->leKeyScience3->setText(QString::number(saveData->getItem(Save::ITEM_ID_SCIENCE_KEY3)));
    m_ui->leKeyClocktower1->setText(QString::number(saveData->getItem(Save::ITEM_ID_CLOCKTOWER_KEY1)));
    m_ui->leKeyClocktower2->setText(QString::number(saveData->getItem(Save::ITEM_ID_CLOCKTOWER_KEY2)));
    m_ui->leKeyClocktower3->setText(QString::number(saveData->getItem(Save::ITEM_ID_CLOCKTOWER_KEY3)));
    m_ui->leItemsER->setText(QString::number(saveData->getItem(Save::ITEM_ID_ENGAGEMENT_RING)));
    m_ui->leItemsIG->setText(QString::number(saveData->getItem(Save::ITEM_ID_INCANDESCENT_GAZE)));
}
